...
c 
        ...
        double precision a(n,m),w(n,m),cd,proj(n,m)
c
...
c                  work arrays:
c
c  w - must be at least n*(n+1) double precision elements long
c 
...
c
     ...
     1      w,lw,ltot)
        implicit double precision (a-h,o-z)
        double precision  a(k,l),x(l,m),b(m,n),c(k,n),w(1)
c 
...
c        call svdpivot(ier,a,k,l,w(iua),w(iva),w(isa),ncolsa,eps2,
c     1      w(iwork),lw1,ltot1)
c 
        ltot=ltot1+iwork
c 
        if(ier .ne. 0) return
...
c 
        eps2=1.0d-13
c 
c        call svdpivot(ier,b,m,n,w(iub),w(ivb),w(isb),ncolsb,eps2,
c     1      w(iwork),lw2,ltot2)
c 
        ii=iwork+ltot2
...
c 
     ...
     1    rnorms,w,ifcheck,errl2,errmax)
        implicit double precision (a-h,o-z)
        dimension w(1)
        double precision x(n,m),c(n,k),a(m,k)
c 
c        This subroutine solves in the least squares sense the
c        matrix equation
c 
...
c  w - the first n*k double precision elements of w contain the
...
c                    work arrays:
c 
c  w - must be at least n*m*16 + 500 double precision elements long
c 
c 
...
c 
     ...
     1    rnorms,w,ifcheck,errl2,errmax)
        ...
        double precision x(m,k),c(n,k),a(n,m),cd,w(1),cd2
c 
c        This subroutine solves in the least squares sense the
c        matrix equation
c 
...
c                    work arrays:
c 
c  w - must be at least n*m*8 + 500 double precision elements long
c 
c 
...
c        n*m*8 + 500 double precision elements of the latter should not be
...
c         Must be at least n*m*8 + 500 double precision elements long
c  rnorms - the normalizing factors in the gram-schmidt process.
c        only the first ncols of them are meaninful, but the array
c        has to be dimensioned by the user to be at least m+1
c        double precision elements long.
...
c         double precision elements of this array should not be changed
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision a(n,m),u(n,1),v(m,1),w(m,1),t(1)
        dimension rnorms(1)
c 
...
c        known, so it should be at least m*n double precision locations long.
c  ncols - the rank of the matrix a to the precision eps. also the
c        second dimension of the matrices u, w on exit
c  rnorms - the normalizing factors in the gram-schmidt process.
c        only the first ncols of them are meaninful, but the array
...
c                      work arrays:
c 
c  v - must be at least n*m double precision locations long
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision u(n,ncols),w(m,ncols),t(ncols,ncols)
        double precision x(m),y(n),work(ncols),d
c 
c         this subroutine uses the QR-type (not exactly QR) decomposition
c         of a matrix to solve in the least squares sense the linear
c         system
...
c                      work arrays:
c 
c  work - must be at least ncols double precision locations long
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision a(n,m),b(m,n),x(1),y(1),z(1)
c 
c       transpose a
c 
        do 1400 i=1,n
...
c  
     ...
     1    eps,v)
        ...
        double precision a(n,m),u(n,1),v(m,1),w(m,1),t(1),rnorms(1)
c 
c        this subroutine decomposes the input matrix a in the
c        form
c               A=U * T * W^*                                       (1)
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision a(n,m),b(n,m),v(m,1),prod
        dimension rnorms(1)
c 
...
c 
        ...
        double precision x(l,m),c(k,n),work(1),
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision a(k,l),b(m,n),c(k,n),d
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision a(k,l),b(m,n),c(l,n),d
c 
c        this subroutine multiplies the matrix a^* by the matrix b,
c        getting the matrix c
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision a(k,l),b(m,n),c(k,m),d
c 
c        this subroutine multiplies the matrix a by the matrix b^*,
c        getting the matrix c
c 
...
c 
        ...
        implicit double precision (a-h,o-z)
        double precision sa(1),sb(1),uacvb(ncolsa,ncolsb)
c 
...
c$$$        subroutine orthom(a,n,work,cond)
c$$$        implicit double precision (a-h,o-z)
c$$$        
c$$$        double precision a(1),work(1)
c$$$c 
c$$$c        this subroutine inverts a user-supplied real matrix
c$$$c        a by means of the gram-schmidt process. in fact, this
c$$$c        is a memory management routine for the routine ortho1,
c$$$c        which performs the actual inversion
c$$$c 
c$$$c               input parameters:
c$$$c 
c$$$c  a - the n*n - matrix to be inverted (destroyed by the
c$$$c         subroutine)
c$$$c  n - the dimensionality of the matrix a
c$$$c 
c$$$c               output parameters:
c$$$c 
c$$$c  a - the inverse of a on input
c$$$c  cond - the estimate of the condition number of the matrix a
c$$$c         that has been inverted
c$$$c 
c$$$c               work array
c$$$c 
c$$$c  work - must be at least  n * (n+1)+2 double precision locations long
c$$$c 
c$$$c 
c$$$c        . . . allocate memory for the matrix inverter
c$$$c 
c$$$        ib=1
c$$$        lb=n*n
c$$$c 
c$$$        iw=ib+lb+1
c$$$c 
c$$$c       invert the matrix
c$$$c 
c$$$        call ortho1(a,work(ib),n,work(iw),cond)
c$$$        return
c$$$        end
c$$$c 
c$$$c 
...
c$$$c 
c$$$c 
c$$$        subroutine ortho1(a,b,n,work,cond)
c$$$        implicit double precision (a-h,o-z)
c$$$        
c$$$        double precision a(n,n),b(n,n),cd,zero,
c$$$     1      work(1)
c$$$c 
c$$$c       set the matrix b to unity
c$$$c 
c$$$        done=1
c$$$        zero=0
c$$$        do 1400 i=1,n
...
c$$$ 1400 continue
c$$$c 
c$$$c        conduct the gram-schmidt process
c$$$c 
c$$$        dnormax=-1
c$$$        dnormin=1.0d23
c$$$        do 4000 i=1,n
c$$$c 
c$$$c       orthogonalize the i-th column to all preceeding ones
c$$$c 
c$$$        if(i .eq. 1) goto 2190
c$$$        do 2180 j=1,i-1
c$$$c 
c$$$        call orthom_scap(a,n,i,j,cd)
c$$$c 
c$$$        do 2170 k=1,n
c$$$        a(i,k)=a(i,k)-a(j,k)*cd
c$$$        b(i,k)=b(i,k)-b(j,k)*cd
c$$$ 2170 continue
c$$$ 2180 continue
c$$$ 2190 continue
c$$$c 
c$$$c        normalize the i-th row
c$$$c 
c$$$        call orthom_scap(a,n,i,i,d)
c$$$c 
c$$$        d=done/sqrt(d)
c$$$        if(dnormax .lt. d) dnormax=d
c$$$        if(dnormin .gt. d) dnormin=d
c$$$        do 2400 j=1,n
c$$$c 
c$$$        a(i,j)=a(i,j)*d
c$$$        b(i,j)=b(i,j)*d
c$$$ 2400 continue
c$$$c 
c$$$c       orthogonalize all subsequent rows to the i-th one
c$$$c 
c$$$        if(i .eq. n) goto 4000
c$$$        do 3000 j=i+1,n
c$$$c 
c$$$        call orthom_scap(a,n,i,j,cd)
c$$$c 
c$$$        do 2800 k=1,n
c$$$        a(j,k)=a(j,k)-cd*a(i,k)
c$$$        b(j,k)=b(j,k)-cd*b(i,k)
c$$$ 2800 continue
c$$$ 3000 continue
c$$$c 
c$$$ 4000 continue
c$$$c 
c$$$        cond=dnormax/dnormin
c$$$c 
c$$$c       now, multiply the abjoint of the resulting
c$$$c       orthogonal matrix by the triangular one,
c$$$c       obtaining the inverse of the original a
c$$$c 
c$$$        do 5000 i=1,n
c$$$        do 4800 j=1,n
c$$$        cd=0
...
c$$$c 
c$$$c 
c$$$        subroutine orthom_matvec(a,n,x,y)
c$$$        implicit double precision (a-h,o-z)
c$$$        
c$$$        double precision a(n,n),x(1),y(1),cd
c$$$c 
c$$$c        apply the matrix a to the vector x obtaining y
c$$$c 
c$$$        do 1400 i=1,n
c$$$        cd=0
c$$$        do 1200 j=1,n
...
c$$$c 
c$$$c 
c$$$        subroutine orthom_scap(a,n,i,j,prod)
c$$$        implicit double precision (a-h,o-z)        
c$$$        dimension a(n,n)
c$$$c 
c$$$        prod=0
c$$$        do 1200 k=1,n
c$$$        prod=prod+a(i,k)*a(j,k)
c$$$ 1200 continue
c$$$        return
c$$$        end
c$$$  
