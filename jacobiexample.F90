      ...
      ! get some of the matlab mex functions
       ...
       real*8, allocatable :: xx(:),xs(:),twhts(:)
       ...
       allocate(ts(nts),xs(nts),twhts(nts),avals0(nts))
       ...
       call jacobi_phase_eval(chebdata,dnu,da,db,nints,ab,avals,
     & psivals,nts,ts,avals0,psivals0)
       ...
       call mxCopyComplex16ToPtr(jacobi1, mxGetPr(plhs(2)),
     & mxGetPi(plhs(2)),nts*(nts-27)) 
       call mxCopyComplex16ToPtr(jacobi2, mxGetPr(plhs(3)),
     & mxGetPi(plhs(3)),nts*(nts-27)) 
