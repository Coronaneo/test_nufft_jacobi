...
%str11='error_cheb';
fprintf('\n');
fprintf('start 3D inv uniform Jacobi polynomial transform test:');
fprintf('\n');
...
%    ncol = size(c,2);
    ...
    tic
    for i = 1:num
    [fun,rank1(ii),rank2(ii),rank3(ii)] = invJPT3D(nts,da,db,tR,mR,tol,1,1);
    end
    timefac(ii)=toc/num;
    %P = U*V.';
    %P = [vals P];
...
%    saveas(bf,'image13a.jpg');
    ...
    legend('N^3 log N','N^3 log^2 N','time app','time fac','Location','NorthWest');
